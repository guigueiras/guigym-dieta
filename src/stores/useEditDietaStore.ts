...
function deepEqualDieta(a: Dieta, b: Dieta): boolean {
  ...
  for (let i = 0; i < a.dias.length; i++) {
    const da = a.dias[i], db = b.dias[i];
...
export const useEditDietaStore = create<EditState>((set, get) => ({
  ...
  salvar: async () => {
    ...
    try {
      ...
      set({
        dietaOriginal: clone(dietaEditada),
        isDirty: false,
        saving: false,
      });
    } catch (err) {
  ...
  addRefeicao: (dia, nome = REFEICAO_PADRAO_NOME) => set((s) => {
    ...
    const editada = mutateDia(s.dietaEditada, dia, (refs) => {
      const list = [...refs, nova].map((r, i) => ({ ...r, ordem: i }));
      return list;
    });
      ...
      refs.map((r) => r.id === refeicaoId
        ? {
            ...
            alimentos: r.alimentos.map((a) =>
              a.id === alimentoNaRefId ? { ...a, quantidade } : a
            ),
          }
        : r
      )
...
export const useEditRefeicoesDoDia = (dia: DiaSemana) =>
  useEditDietaStore(
    useShallow((s) =>
      s.dietaEditada?.dias.find((d) => d.nome === dia)?.refeicoes
        .slice()
...
export const useEditRefeicao = (dia: DiaSemana, refeicaoId: string) =>
  useEditDietaStore(
    useShallow((s) => {
      const r = s.dietaEditada?.dias.find((d) => d.nome === dia)
        ?.refeicoes.find((x) => x.id === refeicaoId);
